 ...
 */
// MUST match slugify() in scripts/gen-sv-audio.ts (which writes the mp3 files).
...
// Each Polish → Swedish pair carries the Swedish pronunciation clip so SELECT /
// ASSIST / TYPE options play it on tap (generated by scripts/gen-sv-audio.ts).
const pair = (source: string, target: string): Item => ({
  source,
  target,
  audio: `/audio/sv/${slugify(target)}.mp3`,
});
